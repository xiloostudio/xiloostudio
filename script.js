 ...
 partitura: [
    ...
    if (telaXilofone) {
        ...
        btnOuvir.onclick = async () => {
            ...
            isPlaying = true;
            btnOuvir.innerText = "⏹ Parar Música";
            
            for (let n of currentSong.partitura) {
                if (!isPlaying) break;
                animar(n.nota);
                tocar(n.nota);
                await esperar(n.dur);
        ...
        btnPraticar.onclick = async () => {
            ...
            isPlaying = true;
            btnPraticar.innerText = "⏹ Parar Prática";
            const blocos = document.querySelectorAll('#partitura-container .nota-block');
            
            for (let i = 0; i < currentSong.partitura.length; i++) {
                if (!isPlaying) break;
                
                blocos.forEach((b, idx) => {
                    b.classList.remove('current');
                    if (idx < i) b.classList.add('played');
                    if (idx === i) b.classList.add('current');
                });
                
                const n = currentSong.partitura[i];
                animar(n.nota);
                tocar(n.nota);
                await esperar(n.dur);
            }
